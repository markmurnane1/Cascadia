...
public class GameController {
	...
	public void shuffleHabitatBag() {
		Collections.shuffle(this.habitatBag);
	}
	...
	public List<Point> validMoves(Tile[][] tileMatrix, ArrayList<Tile> playerTiles) {
		...
		for (Tile t : playerTiles) {
			if (!t.emptyTile()) {
				...
				if (tileMatrix[x - 1][y + 1].emptyTile() && y + 1 < Constants.HEIGHT - 1
						&& x + 1 < Constants.WIDTH - 1) {
					possibleMoves.add(new Point(x - 1, y + 1));
				}
